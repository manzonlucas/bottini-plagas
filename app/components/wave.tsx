...
export const Wave: React.FC<WaveProps> = ({ color, version }) => {
  if (version === 1) {
    return (
      <svg className="absolute bottom-0 w-full h-24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 80" preserveAspectRatio="none">
        <path className={`fill-current ${color}`} fillOpacity="0.99"
  ...
  if (version === 2) {
    return (
      <svg className="absolute top-0 w-full h-24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" preserveAspectRatio="none">
        <path className={`fill-current ${color}`} fillOpacity="0.99"
